...
""".stripMargin
    ...
    assertErrors(List(columnError(3, 0), columnError(5, 0), columnError(6, 0)), source, Map("illegalImports" -> "java.util.List, java.util.Map"))
  }
...
class ImportOrderCheckerTest extends AssertionsForJUnit with CheckerTest {
  ...
  @Test def testNameComparison(): Unit = {
    ...
    assertTrue("import: classes in case-insensitive alphabetical order", checker.compareNames("Classone", "ClassTwo", true) < 0)
    assertTrue("import: packages in alphabetical order", checker.compareNames("package2", "package1", true) > 0)
    assertTrue("import: packages in case-insensitive alphabetical order", checker.compareNames("packagelower2", "packageUpper1", true) < 0)
    ...
    assertTrue("selector: packages in alphabetical order", checker.compareNames("package2", "package1", false) > 0)
  }
  ...
  @Test def testLexicographicNameComparison(): Unit = {
    ...
    assertTrue("import: class before package", checker.compareNames("Class", "package", true) < 0)
    assertTrue("import: classes in alphabetical order", checker.compareNames("Class1", "Class2", true) < 0)
    assertTrue("import: classes in case-sensitive alphabetical order", checker.compareNames("Classone", "ClassTwo", true) > 0)
    assertTrue("import: packages in alphabetical order", checker.compareNames("package2", "package1", true) > 0)
    assertTrue("import: packages in case-sensitive alphabetical order", checker.compareNames("packagelower2", "packageUpper1", true) > 0)
  ...
  @Test def testSubPackage(): Unit = {
    ...
    val expected = List(columnError(3, 0, errorKey = errorKey("wrongOrderInGroup"), args = List("foobar.Bar", "foobar.subpackage.Foo")))
  ...
  @Test def testInGroupOrdering(): Unit = {
    ...
    val expected = List(columnError(3, 21, errorKey = errorKey("wrongOrderInSelector"), args = List("Files", "Paths")))
  ...
  @Test def testImportGrouping(): Unit = {
    ...
    val expected = List(
      ...
      columnError(19, 0, errorKey = errorKey("wrongGroup"), args = List("javax.swing.JTree", "java", "project2"))
    )
